...
export default function ProductCard({ product }) {
  ...
  // 1. We added 'login' here to update the global state!
  const { currentUser, token, login } = useAuth(); 
  const [isWishlisted, setIsWishlisted] = useState(false);

  // Safely check if product is in wishlist on load
  useEffect(() => {
  ...
  const handleWishlistToggle = async (e) => {
    ...
    try {
      ...
      if (response.ok && result.status === "Success") {
        // 2. CRITICAL FIX: Update the global React state so the heart stays filled after refresh!
      ...
      setIsWishlisted(!isWishlisted); // Revert if network crashes
    }
  };
