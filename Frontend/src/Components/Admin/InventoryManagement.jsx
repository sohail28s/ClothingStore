...
export default function InventoryManagement() {
  ...
  const handleSaveStock = async (productId, variantId, sizeId, sku) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${productId}/variants/${variantId}/sizes/${sizeId}/stock`, {
        method: 'PATCH',
  ...
  const handleStatusChange = async (productId, newStatus) => {
    try {
      const token = localStorage.getItem('admin_token');
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${productId}/status`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    ...
    const mappedVariants = product.variants.map(v => {
      ...
      if (v.images) {
        v.images.forEach((img, i) => {
          if (i < 3) images[i] = { file: null, preview: `${import.meta.env.VITE_API_BASE_URL}/${img.replace(/\\/g, '/')}`, original: img };
        });
      }
      return {
  ...
  const submitEdit = async (e) => {
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/products/${formData._id}`, {
        method: 'PUT',
        headers: { 'Authorization': `Bearer ${token}` },
        body: formPayload
      });
                ...
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-white border border-nav-dark shrink-0 p-1">
                    {product.variants[0]?.images[0] ? (
                      <img src={`${import.meta.env.VITE_API_BASE_URL}/${product.variants[0].images[0].replace(/\\/g, '/')}`} alt={product.name} className="w-full h-full object-cover" />
                    ) : (
