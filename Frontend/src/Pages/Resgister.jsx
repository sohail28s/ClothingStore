    ...
    <div className="relative min-h-screen w-full flex items-center justify-center p-4 pt-24 font-central text-nav-dark">
      ...
      <div className="relative z-10 w-full max-w-[500px] bg-[#f4f2ed]/85 backdrop-blur-md border border-nav-dark flex flex-col shadow-2xl">
        <div className="flex border-b border-nav-dark">
          <button onClick={() => setActiveTab('login')} className={`flex-1 py-4 text-sm  tracking-widest font-bold border-r border-nav-dark transition-colors ${activeTab === 'login' ? 'bg-[#f4f2ed] ' : 'bg-transparent text-nav-dark/60 hover:bg-white/40'}`} > Log In </button>
          <button onClick={() => setActiveTab('signup')} className={`flex-1 py-4 text-sm  tracking-widest font-bold transition-colors ${activeTab === 'signup' ? 'bg-[#f4f2ed] ' : 'bg-transparent text-nav-dark/60 hover:bg-white/40'}`} > Sign Up </button>
        </div>

        <div className="p-8 md:p-12 flex flex-col gap-6">
          <h1 className="text-2xl font-bold  tracking-wider mb-2"> {activeTab === 'signup' ? 'Create Your Account' : 'Log In'} </h1>

          {error && <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 text-[10px]  tracking-widest font-bold leading-relaxed">{error}</div>}
          {success && <div className="bg-green-100 border border-green-400 text-green-700 px-4 py-3 text-[10px]  tracking-widest font-bold leading-relaxed">{success}</div>}

          <form onSubmit={activeTab === 'signup' ? handleSignup : handleLogin} className="flex flex-col gap-5" >
            {activeTab === 'signup' && (
              <>
                <input required type="text" name="firstName" placeholder="First Name" value={formData.firstName} onChange={handleInputChange} disabled={isLoading} className="w-full bg-transparent border border-nav-dark px-4 py-3.5 text-xs e tracking-widest outline-none focus:bg-white/50 transition-colors placeholder-nav-dark/60 disabled:opacity-50" />
                <input required type="text" name="lastName" placeholder="Last Name" value={formData.lastName} onChange={handleInputChange} disabled={isLoading} className="w-full bg-transparent border border-nav-dark px-4 py-3.5 text-xs  tracking-widest outline-none focus:bg-white/50 transition-colors placeholder-nav-dark/60 disabled:opacity-50" />
              </>
            )}
            ...
            {activeTab === 'signup' && (
              <label className="flex items-start gap-3 mt-2 cursor-pointer group">
                ...
                <span className="font-ballinger text-[10px]  tracking-widest leading-relaxed font-bold opacity-80 group-hover:opacity-100 transition-opacity disabled:opacity-50">
            ...
            <button type="submit" disabled={isLoading} className={`mt-4 flex w-full h-[52px] border border-nav-dark group ${isLoading ? 'cursor-not-allowed opacity-70' : 'cursor-pointer'}`}>
              <div className="flex-1 bg-[#f4f2ed] flex items-center justify-center font-bold  tracking-widest text-sm transition-colors group-hover:bg-white">
            ...
            <p className="text-center font-ballinger text-[9px]  tracking-widest mt-6 opacity-70 leading-relaxed font-bold px-4">
