...
export default function Checkout() {
  ...
  const SHIPPING_COST = 300;
  // This logic is correct: 10k or more gets free shipping.
  const finalTotal = cartTotal >= 10000 ? cartTotal : cartTotal + SHIPPING_COST;
  ...
  useEffect(() => {
    ...
    if (activeUser) {
      const defaultAddress = activeUser.addresses && activeUser.addresses.length > 0 ? activeUser.addresses[0] : {};
      setFormData(prev => ({
        ...prev,
        email: activeUser.email || prev.email,
        firstName: activeUser.firstName || defaultAddress.firstName || prev.firstName,
          ...
          <div className="w-full max-w-[600px]">
            ...
            <form onSubmit={handleCheckout} className="flex flex-col gap-8">
              ...
              <div>
                ...
                <input required type="email" name="email" value={formData.email} onChange={handleInputChange} placeholder="Email address" className="w-full p-3 border border-gray-300 rounded-[4px] focus:ring-2 focus:ring-nav-dark outline-none transition-all placeholder-gray-400" />
                <label className="flex items-center gap-2 mt-3 cursor-pointer">
                  <input type="checkbox" name="emailOffers" checked={formData.emailOffers} onChange={handleInputChange} className="w-4 h-4 accent-nav-dark" />
                  <span className="text-sm text-gray-600">Email me with news and offers</span>
                </label>
              </div>
              ...
              <button type="submit" disabled={isSubmitting} className={`w-full text-white p-4 rounded-[4px] font-central text-lg uppercase tracking-widest font-bold transition-colors shadow-md ${isSubmitting ? 'bg-gray-400 cursor-not-allowed' : 'bg-nav-dark hover:bg-black'}`} >
                {isSubmitting ? 'Processing...' : 'Complete Order'}
              </button>
            </form>
          </div>
        ...
        <div className="w-full lg:w-[45%] xl:w-[40%] bg-[#fafafa] lg:border-l border-gray-200 pt-10 pb-20 px-4 md:px-12 lg:pr-20 xl:pr-32 lg:pl-16 lg:mt-[80px]">
          <div className="w-full max-w-[500px]">
            ...
            <div className="border-t border-gray-300 py-4 flex flex-col gap-3 text-sm">
              ...
              <div className="flex justify-between">
                <span className="text-gray-600">Shipping</span>
                <span className="font-medium text-nav-dark">
                  {cartTotal >= 10000 ? 'Free' : `PKR ${SHIPPING_COST.toLocaleString()}`}
                </span>
              </div>
            </div>
