...
//   // CRITICAL: We now pull in the 'token' from useAuth so we can authorize the API call
...
//     firstName: '',
//     lastName: '',
//     addressLine: '', // Note: Backend expects 'address'
//     city: '',
//     postcode: '',    // Note: Backend expects 'postalCode'
//     country: 'Pakistan'
//   });
...
//       // 2. REAL DB LOAD: Load Addresses directly from the currentUser profile we got during login
...
//   // --- ACCOUNT LOGIC (Still LocalStorage for now) ---
...
//     e.preventDefault();
//     const updatedUser = { ...currentUser, ...accountForm };
//     login(token, updatedUser); 
//     setIsEditingAccount(false);
//   };

//   // ==========================================
//   // --- REAL ADDRESS LOGIC (API INTEGRATION) ---
//   // ==========================================
...
//     try {
//       // 1. Real Fetch Call to your backend
...
//         },
//         // 2. Map frontend state names to backend schema names perfectly
//         body: JSON.stringify({
//           label: newAddress.label,
//           firstName: newAddress.firstName,
//           lastName: newAddress.lastName,
//           address: newAddress.addressLine, // Mapping addressLine -> address
//           city: newAddress.city,
//           postalCode: newAddress.postcode, // Mapping postcode -> postalCode
...
//       // 3. Success! The backend returns the full, newly updated array of addresses
//       // We update our local state instantly so the user sees it without refreshing
//       setSavedAddresses(result.data);
      
//       // 4. Update the global context so the whole app knows about the new address
...
//       alert(error.message); // Simple alert for now, you can upgrade to a toast notification later
//     } finally {
//       setIsSavingAddress(false);
//     }
//   };

//   // Note: We will hook this up to a real DELETE API later!
...
//                           <input required type="text" name="firstName" value={accountForm.firstName} onChange={handleAccountInputChange} className="w-full p-4 border border-gray-300 outline-none focus:border-nav-dark font-ballinger text-sm" />
//                         </div>
//                         <div>
//                           <label className="font-central text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-2 block">Last Name</label>
//                           <input required type="text" name="lastName" value={accountForm.lastName} onChange={handleAccountInputChange} className="w-full p-4 border border-gray-300 outline-none focus:border-nav-dark font-ballinger text-sm" />
//                         </div>
//                       </div>
//                       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//                         <div>
//                           <label className="font-central text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-2 block">Email Address</label>
//                           <input required type="email" name="email" value={accountForm.email} onChange={handleAccountInputChange} className="w-full p-4 border border-gray-300 outline-none focus:border-nav-dark font-ballinger text-sm" />
//                         </div>
//                         <div>
//                           <label className="font-central text-[10px] uppercase tracking-widest font-bold text-gray-500 mb-2 block">Phone Number</label>
//                           <input type="tel" name="phone" value={accountForm.phone} onChange={handleAccountInputChange} placeholder="Optional" className="w-full p-4 border border-gray-300 outline-none focus:border-nav-dark font-ballinger text-sm" />
//                         </div>
//                       </div>
//                       <div className="flex flex-col gap-4 mt-4 border-t border-gray-200 pt-6">
//                         <label className="flex items-start gap-4 cursor-pointer group">
//                           <input type="checkbox" name="emailNewsletter" checked={accountForm.emailNewsletter} onChange={handleAccountInputChange} className="mt-1 w-5 h-5 rounded-none border border-nav-dark accent-nav-dark bg-transparent shrink-0" />
...
//                         </label>
//                         <label className="flex items-start gap-4 cursor-pointer group">
//                           <input type="checkbox" name="pushNotification" checked={accountForm.pushNotification} onChange={handleAccountInputChange} className="mt-1 w-5 h-5 rounded-none border border-nav-dark accent-nav-dark bg-transparent shrink-0" />
//                           <span className="font-ballinger text-[12px] text-gray-600 leading-relaxed group-hover:text-nav-dark transition-colors">
...
//                         <button type="submit" className="flex-1 bg-nav-dark text-white p-4 font-central text-sm font-bold uppercase tracking-widest hover:bg-black transition-colors">
//                           Save Changes
//                         </button>
//                         <button type="button" onClick={() => setIsEditingAccount(false)} className="flex-1 border border-nav-dark text-nav-dark p-4 font-central text-sm font-bold uppercase tracking-widest hover:bg-gray-50 transition-colors">
//                           Cancel
//                         </button>
//                       </div>
//                     </form>
...
//                         // Map using addr._id if it exists from MongoDB, otherwise fallback to index/id
...
//                           {/* We read from addr.address because that's what the DB returns */}
